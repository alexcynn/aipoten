...
export default function ParentingBoardPage() {
  ...
  const fetchPosts = async () => {
    try {
      const response = await fetch('/api/boards/parenting/posts')
      if (response.ok) {
        ...
        <div className="px-4 py-6 sm:px-0">
          ...
          <div className="mb-8">
            <div className="flex justify-between items-center">
              <div>
                <h1 className="text-3xl font-bold text-gray-900">육아소통</h1>
                <p className="mt-2 text-gray-600">
                  육아 경험과 팁을 나누며 소통하는 공간입니다
                </p>
              </div>
              {session && (
                <Link
                  href="/boards/new?board=parenting"

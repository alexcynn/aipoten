# aipoten 업무 플로우

## 목차
1. [부모 사용자 플로우](#부모-사용자-플로우)
2. [치료사 플로우](#치료사-플로우)
3. [관리자 플로우](#관리자-플로우)
4. [시스템 간 연동 플로우](#시스템-간-연동-플로우)

---

## 부모 사용자 플로우

### 1. 회원가입 및 온보딩

```
시작
  ↓
회원가입 (이메일/소셜 로그인)
  ↓
약관 동의 (서비스 이용약관, 개인정보 처리방침, 아동 정보 수집 동의)
  ↓
아이 프로필 등록
  [기본 정보]
  - 이름 (닉네임 가능)
  - 생년월일
  - 성별

  [출생 정보]
  - 출산 주수 (선택)
  - 출생 시 몸무게 (선택)

  [현재 신체 정보]
  - 현재 키 (선택)
  - 현재 몸무게 (선택)

  [의료 정보] (선택)
  - 병력/수술력
  - 가족력
  - 치료력

  [기타]
  - 특이사항 (선택)
  ↓
첫 발달 체크 안내
  ↓
온보딩 완료 → 메인 화면
```

**주요 화면:**
- 회원가입 화면
- 아이 프로필 등록 화면
- 온보딩 튜토리얼

### 2. 발달 체크 플로우

#### 2.1 비로그인 발달 체크 플로우 (체험판)

```
메인 화면
  ↓
[발달 체크하기] 선택
  ↓
발달 체크 소개 화면
  - 6가지 영역 설명 (대근육, 소근육, 인지, 언어, 사회성, 정서)
  - 체험판 안내: "로그인 없이 1개 영역 무료 체크 가능"
  - 전체 진단을 위한 로그인 유도
  ↓
[체험하기] 또는 [로그인하고 전체 진단] 선택
  ↓
[체험하기 선택 시]
  ↓
체험 영역 선택
  - 6개 영역 중 1개만 선택 가능
  - 인기 영역 표시 (예: 언어 발달)
  ↓
아이 기본 정보 입력 (간소화)
  - 개월 수 (필수)
  - 성별 (남아/여아) (필수)
  - 키 (cm) (선택)
  - 몸무게 (kg) (선택)
  ↓
선택한 영역 질문 응답 (10문항)
  - 진행바 표시
  - 이전/다음 버튼
  ↓
체험 결과 확인
  - 선택한 영역 점수
  - 간단한 평가
  - "전체 진단을 위해 로그인하세요" 안내
  ↓
[회원가입/로그인] 유도
  - "전체 6가지 영역 진단하기"
  - "발달 기록 저장 및 추적하기"
  - "맞춤 놀이영상 보기" (로그인 필수)
```

#### 2.2 로그인 후 발달 체크 플로우 (전체 진단)

```
메인 화면 (로그인 상태)
  ↓
[발달 체크하기] 선택
  ↓
사용자 상태 확인
  ↓
┌─────────────────────────────────────────┐
│ 아이 정보 유무 판단                        │
└─────────────────────────────────────────┘
         │
         ├──→ [아이 정보 없음]
         │      ↓
         │    아이 프로필 등록 안내
         │      ↓
         │    아이 정보 입력 화면
         │      [기본 정보]
         │      - 이름 (닉네임 가능)
         │      - 생년월일 (자동으로 개월 수 계산)
         │      - 성별
         │
         │      [출생 정보]
         │      - 출산 주수 (예: 38주, 40주) (선택)
         │      - 출생 시 몸무게 (kg) (선택)
         │
         │      [현재 신체 정보]
         │      - 현재 키 (cm) (선택)
         │      - 현재 몸무게 (kg) (선택)
         │
         │      [의료 정보] (선택사항)
         │      - 병력/수술력 (텍스트 입력 또는 선택)
         │        예: 심장 질환, 호흡기 질환, 수술 이력 등
         │      - 가족력 (텍스트 입력 또는 선택)
         │        예: 발달 지연, 유전 질환 등
         │      - 치료력 (현재 받고 있는 치료)
         │        예: 언어치료, 작업치료, 물리치료 등
         │
         │      [기타]
         │      - 특이사항 (자유 텍스트)
         │      ↓
         │    아이 정보 저장
         │      ↓
         │    ─────────────┐
         │                 │
         └──→ [아이 정보 있음]
                  ↓
         ┌────────┴────────┐
         │                 │
    [단일 아이]      [다자녀 있음]
         │                 │
         │                 ↓
         │          아이 선택 화면
         │            - 아이별 카드 표시
         │            - 이름, 나이, 마지막 체크일
         │            - [아이 추가하기] 버튼
         │                 │
         └─────────────────┘
                  ↓
         발달 체크 시작 화면
           - 선택한 아이 정보 확인
           - 전체 6개 영역 안내
           - 예상 소요 시간 (약 15분)
           - [시작하기] 버튼
                  ↓
         영역별 질문 응답 (6개 영역 × 10문항 = 60문항)
           1. 대근육 운동 발달 (10문항)
           2. 소근육 운동 발달 (10문항)
           3. 인지 발달 (10문항)
           4. 언어 발달 (10문항)
           5. 사회성 발달 (10문항)
           6. 정서 발달 (10문항)
                  ↓
         각 질문 화면
           - 진행바 (예: 15/60)
           - 현재 영역 표시
           - 질문 텍스트
           - 응답 옵션 (예: 예/아니오/때때로)
           - [이전] [다음] 버튼
           - [나중에 계속하기] (중간 저장)
                  ↓
         중간 저장 기능
           - 자동 저장 (5문항마다)
           - 수동 저장 가능
           - 재진입 시 이어하기 옵션
                  ↓
         모든 질문 완료 (60문항)
                  ↓
         결과 분석 중... (로딩 화면)
           - "아이의 발달을 분석하고 있어요"
           - 분석 애니메이션 (약 3-5초)
                  ↓
         [백엔드 처리 - 사용자에게 비노출]
         ┌─────────────────────────────────────────┐
         │ 1. 발달 체크 결과 저장                  │
         │ 2. 영역별 점수 및 단계(level) 계산      │
         │ 3. 추천 영상 자동 생성 프로세스 시작    │
         │    ↓                                    │
         │    영역별 추천 알고리즘 실행:           │
         │    - 주의/관찰 필요 영역 (점수 < 60점)  │
         │      → 해당 영역 영상 10개 선정         │
         │    - 양호 영역 (60-79점)                │
         │      → 해당 영역 영상 5개 선정          │
         │    - 우수 영역 (80점 이상)              │
         │      → 해당 영역 심화 영상 5개 선정     │
         │    ↓                                    │
         │ 4. 아이 연령에 맞는 영상 필터링         │
         │    (아이 개월수 ±3개월 범위)            │
         │ 5. 우선순위 정렬 (priority 높은 순)     │
         │ 6. 추천 영상 목록 DB 저장               │
         │    → recommended_videos 테이블에 insert │
         │ 7. 리포트 화면에 표시할 상위 3개 선정   │
         └─────────────────────────────────────────┘
                  ↓
         발달 리포트 화면
           ┌─────────────────────────┐
           │ 종합 발달 상태           │
           │ - 전체 평균 점수         │
           │ - 종합 발달 단계 표시    │
           └─────────────────────────┘
           ┌─────────────────────────┐
           │ 영역별 레이더 차트       │
           │ - 6개 영역 시각화        │
           │ - 4단계 구분선 표시      │
           └─────────────────────────┘
           ┌─────────────────────────────────────────┐
           │ 영역별 발달 단계 (4단계 구분)            │
           │                                          │
           │ 🟢 대근육: 우수 (80-100점)               │
           │    "연령에 적합한 대근육 발달을 보입니다"│
           │                                          │
           │ 🟢 소근육: 우수 (80-100점)               │
           │    "손과 손가락 움직임이 잘 발달했습니다"│
           │                                          │
           │ 🟡 인지: 양호 (60-79점)                  │
           │    "인지 발달이 적절히 진행 중입니다"    │
           │                                          │
           │ 🟠 언어: 주의 (40-59점)                  │
           │    "언어 발달에 좀 더 관심이 필요합니다" │
           │    [권장 놀이 보기] [전문가 상담]        │
           │                                          │
           │ 🟢 사회성: 우수 (80-100점)               │
           │    "또래와의 상호작용이 원활합니다"      │
           │                                          │
           │ 🟢 정서: 우수 (80-100점)                 │
           │    "감정 표현과 조절이 잘 되고 있습니다" │
           └─────────────────────────────────────────┘

           ┌─────────────────────────────────────────┐
           │ 발달 단계 기준                           │
           │                                          │
           │ 🟢 우수 (80-100점)                       │
           │    - 해당 영역이 매우 잘 발달되었습니다  │
           │    - 현재 수준을 유지하며 지속 발달      │
           │                                          │
           │ 🟡 양호 (60-79점)                        │
           │    - 정상 범위의 발달을 보이고 있습니다  │
           │    - 꾸준한 놀이 활동 권장               │
           │                                          │
           │ 🟠 주의 (40-59점)                        │
           │    - 발달 촉진 활동이 필요합니다         │
           │    - 맞춤 놀이 및 전문가 상담 고려       │
           │                                          │
           │ 🔴 관찰 필요 (0-39점)                    │
           │    - 전문가 평가를 권장합니다            │
           │    - 조기 개입이 중요합니다              │
           └─────────────────────────────────────────┘
           ┌─────────────────────────────────────────┐
           │ 🎯 OO이를 위한 맞춤 놀이영상이         │
           │    자동으로 준비되었습니다!             │
           │                                         │
           │ 🟠 언어 발달 집중 놀이 (10개)           │
           │ 🟡 인지 발달 유지 놀이 (5개)            │
           │ 🟢 대근육 심화 놀이 (5개)               │
           │                                         │
           │ 총 20개의 영상이 추천되었습니다         │
           └─────────────────────────────────────────┘
           ┌─────────────────────────────────────────┐
           │ 주요 추천 영상 미리보기 (상위 3개)      │
           │                                         │
           │ 🟠 언어 발달 놀이 (주의 영역 우선)      │
           │ ┌───────────────────────────┐           │
           │ │ [썸네일]                  │           │
           │ │ 말놀이 동요 따라하기      │           │
           │ │ 24-30개월 | 언어 | 쉬움   │           │
           │ │ [▶ 보기]                  │           │
           │ └───────────────────────────┘           │
           │                                         │
           │ ┌───────────────────────────┐           │
           │ │ [썸네일]                  │           │
           │ │ 그림책 읽어주기           │           │
           │ │ 24-36개월 | 언어 | 쉬움   │           │
           │ │ [▶ 보기]                  │           │
           │ └───────────────────────────┘           │
           │                                         │
           │ ┌───────────────────────────┐           │
           │ │ [썸네일]                  │           │
           │ │ 소리 듣고 따라하기        │           │
           │ │ 20-30개월 | 언어 | 보통   │           │
           │ │ [▶ 보기]                  │           │
           │ └───────────────────────────┘           │
           │                                         │
           │ ┌─────────────────────────┐             │
           │ │ [모든 추천 영상 보기] 🎬│             │
           │ └─────────────────────────┘             │
           └─────────────────────────────────────────┘
                  ↓
         액션 선택
           ├──→ [▶ 보기] (개별 영상)
           │     클릭 시 영상 상세 화면으로 이동
           │     (바로 재생 가능)
           │
           ├──→ [모든 추천 영상 보기] 🎬 (메인 동선)
           │     ↓
           │     마이페이지 > 추천영상 페이지로 이동
           │     ↓
           │     ┌─────────────────────────────────┐
           │     │ OO이를 위한 맞춤 추천 영상      │
           │     │ (이번 체크 기준 - 2025-09-28)   │
           │     ├─────────────────────────────────┤
           │     │ 🟠 주의 영역: 언어 발달 (10개)  │
           │     │ [영상1] [영상2] ... [영상10]    │
           │     │                                 │
           │     │ 🟡 양호 영역: 인지 발달 (5개)   │
           │     │ [영상11] [영상12] ... [영상15]  │
           │     │                                 │
           │     │ 🟢 우수 영역: 대근육 등 (5개)   │
           │     │ [영상16] [영상17] ... [영상20]  │
           │     └─────────────────────────────────┘
           │
           ├──→ [치료사 찾기]
           │     (해당 영역 전문 치료사 추천)
           │
           ├──→ [리포트 저장/공유]
           │     (PDF 다운로드, 카카오톡 공유)
           │
           └──→ [메인으로 돌아가기]
```

#### 2.3 발달 기록 이력 확인

```
마이페이지
  ↓
[발달 기록] 메뉴
  ↓
아이 선택 (다자녀인 경우)
  ↓
발달 체크 이력 목록
  - 날짜별 체크 결과
  - 종합 점수 표시
  ↓
특정 체크 결과 선택
  ↓
과거 리포트 상세 확인
  ↓
[현재와 비교하기]
  ↓
발달 추이 그래프
  - 시간에 따른 영역별 점수 변화
  - 성장 곡선
  - 마일스톤 표시
```

**주요 화면:**
- 발달 체크 소개 화면
- 체험판 영역 선택 화면
- 아이 기본 정보 입력 화면 (비로그인)
- 아이 프로필 등록 화면 (로그인)
- 아이 선택 화면 (다자녀)
- 질문 응답 화면 (진행바 포함)
- 발달 리포트 화면 (종합)
- 발달 추이 그래프 화면

**알림:**
- 정기 체크 알림 (1개월마다)
- 발달 지연 의심 시 전문가 상담 권장 알림
- 체험판 사용자에게 회원가입 유도 알림

**제한 사항:**
| 기능 | 비로그인 (체험판) | 로그인 (전체) |
|------|-----------------|--------------|
| 진단 영역 | 1개 영역만 | 전체 6개 영역 |
| 결과 저장 | 불가 | 가능 |
| 발달 추이 | 불가 | 가능 |
| 놀이영상 | 불가 (로그인 유도) | 맞춤 추천 제공 |
| 치료사 연결 | 불가 | 가능 |

### 3. 놀이영상 플로우

#### 3.1 부모 사용자 - 놀이영상 시청 플로우

```
메인 화면 또는 발달 리포트 (로그인 상태)
  ↓
[놀이영상] 메뉴 선택
  ↓
놀이영상 홈 화면
  ┌─────────────────────────────────┐
  │ OO이를 위한 맞춤 추천 영상      │
  │ (발달 체크 결과 기반)           │
  ├─────────────────────────────────┤
  │ 🟠 언어 발달을 위한 놀이        │
  │ - [영상1] 말놀이 동요 따라하기  │
  │ - [영상2] 그림책 읽어주기       │
  │ - [영상3] 소리 듣고 따라하기    │
  ├─────────────────────────────────┤
  │ 🟢 대근육 발달 유지 놀이        │
  │ - [영상4] 신나는 체조 시간      │
  │ - [영상5] 공놀이 하기           │
  └─────────────────────────────────┘
  ┌─────────────────────────────────┐
  │ 카테고리별 탐색                 │
  │ - 대근육 놀이                   │
  │ - 소근육 놀이                   │
  │ - 언어 놀이                     │
  │ - 인지 놀이                     │
  │ - 사회성 놀이                   │
  │ - 정서 놀이                     │
  └─────────────────────────────────┘
  ┌─────────────────────────────────┐
  │ 인기 영상 / 새로운 영상         │
  └─────────────────────────────────┘
  ↓
영상 선택
  ↓
영상 상세 화면
  ┌─────────────────────────────────┐
  │ [썸네일 이미지]                 │
  │                                 │
  │ 영상 제목                       │
  │ - 출처: YouTube / 자체 제작     │
  │ - 대상 연령: 24-30개월          │
  │ - 발달 영역: 언어 발달          │
  │ - 놀이 시간: 10분               │
  │ - 난이도: 쉬움                  │
  │ - 조회수: 1,234회               │
  │                                 │
  │ 준비물:                         │
  │ - 인형, 그림책                  │
  │                                 │
  │ 놀이 설명:                      │
  │ "아이와 함께 그림책을 보며..."  │
  │                                 │
  │ [▶ 영상 재생하기]               │
  │ [🔖 북마크] [✓ 완료] [📤 공유]  │
  └─────────────────────────────────┘
  ↓
[▶ 영상 재생하기] 클릭
  ↓
영상 재생 화면
  ┌─────────────────────────────────┐
  │                                 │
  │   YouTube / 임베디드 플레이어   │
  │   (외부 링크 재생)              │
  │                                 │
  │   - 유튜브 영상: iframe 삽입    │
  │   - 네이버TV: 임베디드 코드     │
  │   - 기타 플랫폼: 지원 형식      │
  │                                 │
  └─────────────────────────────────┘
  ↓
재생 중 / 재생 완료
  ↓
재생 완료 시 팝업
  "놀이를 완료하셨나요?"
  [✓ 완료 체크] [나중에]
  ↓
[완료 체크 시]
  - 시청 기록 저장
  - 완료 횟수 증가
  - 다음 권장 놀이 추천
  ↓
관련 영상 추천
  "이런 놀이는 어때요?"
  - 같은 영역 다른 난이도 영상
  - 다음 단계 영상
  - 유사한 놀이 영상
```

**주요 화면:**
- 놀이영상 홈 (맞춤 추천 + 카테고리)
- 카테고리별 영상 목록
- 영상 상세 화면
- 영상 재생 화면 (임베디드 플레이어)
- 나의 북마크 화면
- 시청 기록 화면

**영상 접근:**
- 모든 영상 무료 제공 (로그인 필수)
- 제한 없는 시청

#### 3.2 관리자 - 놀이영상 등록 플로우

```
관리자 대시보드
  ↓
[콘텐츠 관리] > [놀이영상 관리] 선택
  ↓
놀이영상 목록 화면
  - 등록된 영상 목록
  - 검색 / 필터링
  - [+ 새 영상 등록] 버튼
  ↓
[+ 새 영상 등록] 클릭
  ↓
영상 등록 폼
  ┌─────────────────────────────────────────┐
  │ [1단계] 영상 소스 정보                  │
  │                                         │
  │ 영상 출처 선택:                         │
  │ ○ YouTube                               │
  │ ○ 네이버TV                              │
  │ ○ 카카오TV                              │
  │ ○ 기타 URL                              │
  │                                         │
  │ 영상 URL 입력: (필수)                   │
  │ [https://www.youtube.com/watch?v=...]  │
  │                                         │
  │ [미리보기] 버튼                         │
  │ → 클릭 시 영상 로딩 및 썸네일 자동 추출│
  │                                         │
  │ ┌───────────────────────────┐           │
  │ │ 영상 미리보기             │           │
  │ │ (임베디드 플레이어)       │           │
  │ └───────────────────────────┘           │
  │                                         │
  │ 썸네일 이미지:                          │
  │ ○ 자동 추출 (YouTube 썸네일)            │
  │ ○ 직접 업로드                           │
  │ [파일 선택]                             │
  │                                         │
  └─────────────────────────────────────────┘
  ↓
  ┌─────────────────────────────────────────┐
  │ [2단계] 영상 메타데이터                 │
  │                                         │
  │ 영상 제목: (필수)                       │
  │ [말놀이 동요 따라하기]                  │
  │                                         │
  │ 영상 설명: (필수)                       │
  │ [아이와 함께 동요를 들으며...]          │
  │                                         │
  │ 발달 영역: (필수, 다중 선택 가능)       │
  │ ☑ 대근육  ☐ 소근육                      │
  │ ☐ 인지    ☑ 언어                        │
  │ ☐ 사회성  ☐ 정서                        │
  │                                         │
  │ 대상 연령: (필수)                       │
  │ 최소: [24] 개월  최대: [36] 개월        │
  │                                         │
  │ 난이도: (필수)                          │
  │ ○ 쉬움  ○ 보통  ○ 어려움                │
  │                                         │
  │ 놀이 시간:                              │
  │ [10] 분                                 │
  │                                         │
  │ 준비물: (선택)                          │
  │ [인형, 그림책, 색연필]                  │
  │                                         │
  │ 카테고리: (다중 선택)                   │
  │ ☑ 언어놀이                              │
  │ ☐ 신체놀이                              │
  │ ☐ 감각놀이                              │
  │                                         │
  │ 태그: (선택)                            │
  │ [동요, 따라하기, 말하기, 듣기]          │
  │                                         │
  └─────────────────────────────────────────┘
  ↓
  ┌─────────────────────────────────────────┐
  │ [3단계] 콘텐츠 상세 정보                │
  │                                         │
  │ 놀이 방법: (상세 설명)                  │
  │ [에디터]                                │
  │ 1. 아이와 함께 앉아서...                │
  │ 2. 동요를 틀어주고...                   │
  │ 3. 율동을 함께 따라해보세요             │
  │                                         │
  │ 기대 효과:                              │
  │ [에디터]                                │
  │ - 언어 발달 촉진                        │
  │ - 듣기 능력 향상                        │
  │ - 모방 능력 발달                        │
  │                                         │
  │ 주의사항:                               │
  │ [아이가 거부하면 강요하지 마세요]       │
  │                                         │
  └─────────────────────────────────────────┘
  ↓
  ┌─────────────────────────────────────────┐
  │ [4단계] 공개 설정                       │
  │                                         │
  │ 공개 상태:                              │
  │ ○ 즉시 공개                             │
  │ ○ 예약 공개: [날짜/시간 선택]           │
  │ ○ 비공개 (검토 중)                      │
  │                                         │
  │ 추천 우선순위:                          │
  │ [5] (1-10, 높을수록 우선 노출)          │
  │                                         │
  └─────────────────────────────────────────┘
  ↓
[미리보기] 또는 [저장]
  ↓
[미리보기 선택 시]
  - 사용자 화면과 동일하게 미리보기
  - 영상 재생 테스트
  - 링크 유효성 확인
  ↓
[저장 선택 시]
  ↓
유효성 검증
  - 필수 항목 입력 확인
  - URL 유효성 체크
  - 연령 범위 적합성 확인
  ↓
데이터베이스 저장
  - videos 테이블에 저장
  - video_url: 외부 링크 (YouTube 등)
  - video_platform: 플랫폼 정보
  - embed_code: 임베디드 코드
  ↓
저장 완료
  "영상이 성공적으로 등록되었습니다"
  [목록으로] [계속 등록하기]
```

**관리자 주요 화면:**
- 영상 목록 관리 화면
- 영상 등록 폼 (4단계)
- 영상 수정 화면
- 영상 통계 대시보드
  - 조회수
  - 북마크 수
  - 완료 횟수
  - 발달 영역별 인기 영상

**영상 소스 관리:**
- 외부 링크 저장 (YouTube, 네이버TV 등)
- 임베디드 코드 자동 생성
- 썸네일 자동 추출 또는 업로드
- 링크 유효성 주기적 체크
- 링크 만료 시 알림

**지원 플랫폼:**
- YouTube
- 네이버TV
- 카카오TV
- Vimeo
- 기타 임베디드 지원 플랫폼

### 4. 치료사 방문 세션 매칭 및 결제 플로우

#### 4.0 치료사 - 주간 스케줄 설정 (사전 작업)

```
치료사 대시보드
  ↓
[스케줄 관리] 메뉴
  ↓
주간 가능 시간 설정
  ┌─────────────────────────────────────┐
  │ 주간 스케줄 설정                    │
  ├─────────────────────────────────────┤
  │ 월요일:                             │
  │ ☑ 09:00-10:00  ☑ 10:00-11:00       │
  │ ☐ 11:00-12:00  ☐ 12:00-13:00       │
  │ ☑ 14:00-15:00  ☑ 15:00-16:00       │
  │ ☑ 16:00-17:00  ☐ 17:00-18:00       │
  │                                     │
  │ 화요일:                             │
  │ ☑ 09:00-10:00  ☑ 10:00-11:00       │
  │ ...                                 │
  │                                     │
  │ 수요일:                             │
  │ ☐ 전체 휴무                         │
  │                                     │
  │ 목요일 ~ 일요일:                    │
  │ [시간 선택...]                      │
  │                                     │
  │ ⚠️ 참고사항:                        │
  │ - 1회 세션: 50분                    │
  │ - 세션 간 휴식: 10분 (자동 적용)    │
  │ - 예: 14:00 세션 시 15:00까지 블록  │
  └─────────────────────────────────────┘
  ↓
[저장하기]
  ↓
스케줄 저장 완료
  "주간 스케줄이 저장되었습니다"

  ↓
특정 날짜 예외 설정 (선택)
  ┌─────────────────────────────────────┐
  │ 예외 일정 등록                      │
  ├─────────────────────────────────────┤
  │ 날짜 선택: 2025-10-10 (화)         │
  │                                     │
  │ ○ 이 날은 휴무                      │
  │ ○ 특정 시간만 가능:                 │
  │   ☑ 14:00-15:00  ☑ 15:00-16:00     │
  │                                     │
  │ [추가하기]                          │
  └─────────────────────────────────────┘
```

#### 4.1 부모 - 치료사 검색 및 예약 가능 시간 확인

```
메인 화면 또는 발달 리포트
  ↓
[치료사 찾기] 선택
  ↓
치료 분야 선택
  - 언어치료
  - 작업치료
  - 물리치료
  - 심리치료
  - 발달놀이치료
  - 감각통합치료
  ↓
필터 설정
  - 지역 (시/구/동) - 방문 가능 지역
  - 경력 (1년 이상, 5년 이상, 10년 이상)
  - 전문 분야 (발달지연, ADHD, 자폐 등)
  - 세션 비용 범위
  ↓
치료사 목록 확인
  - AI 추천 치료사 (아이 발달 데이터 기반)
  - 리뷰 평점 높은 순
  - 거리 가까운 순
  ↓
치료사 프로필 확인
  ┌─────────────────────────────────────┐
  │ 치료사 프로필                       │
  ├─────────────────────────────────────┤
  │ - 이름 및 전문 분야                 │
  │ - 경력 (예: 5년)                    │
  │ - 자격증                            │
  │ - 방문 가능 지역                    │
  │ - 세션 시간: 50분                   │
  │ - 세션 비용: 80,000원/회            │
  │ - 평점: 4.8/5.0 (리뷰 32개)        │
  │ - 전문 분야: 언어 발달 지연         │
  │                                     │
  │ [예약 가능한 시간 보기]             │
  └─────────────────────────────────────┘
  ↓
[예약 가능한 시간 보기] 클릭
  ↓
실시간 예약 가능 시간 캘린더 표시
  ┌─────────────────────────────────────┐
  │ 예약 가능 시간 (2주 범위)           │
  ├─────────────────────────────────────┤
  │ 📅 2025년 10월                      │
  │                                     │
  │ 월  화  수  목  금  토  일          │
  │ 5   6   7   8   9   10  11         │
  │                                     │
  │ 10/5 (토) - 선택 가능               │
  │ ✅ 09:00-10:00                      │
  │ ✅ 10:00-11:00                      │
  │ ❌ 11:00-12:00 (예약됨)             │
  │ ✅ 14:00-15:00                      │
  │ ✅ 15:00-16:00                      │
  │ ❌ 16:00-17:00 (예약됨)             │
  │                                     │
  │ 10/6 (일) - 선택 가능               │
  │ ✅ 09:00-10:00                      │
  │ ✅ 10:00-11:00                      │
  │ ...                                 │
  │                                     │
  │ ⚠️ 주의사항:                        │
  │ - 세션 시간: 50분                   │
  │ - 세션 후 1시간은 예약 불가         │
  │   (예: 14:00 예약 시, 15:00 자동 블록)│
  └─────────────────────────────────────┘
  ↓
원하는 시간 선택
  ↓
세션 정보 입력
  ┌─────────────────────────────────────┐
  │ 예약 정보 확인                      │
  ├─────────────────────────────────────┤
  │ 선택한 일시:                        │
  │ 📅 2025-10-05 (토) 14:00           │
  │                                     │
  │ 아이 선택: [OO이 (24개월)]         │
  │                                     │
  │ 방문 주소 입력:                     │
  │ [서울시 강남구 ...]                 │
  │ ☑ 치료사 방문 가능 지역 내          │
  │                                     │
  │ 아이 정보 공유 동의:                │
  │ ☑ 발달 체크 결과 공유               │
  │ ☑ 의료 정보 공유                    │
  │                                     │
  │ 특이사항 및 요청사항:               │
  │ [자유 입력]                         │
  └─────────────────────────────────────┘
  ↓
[예약 신청하기] 클릭
  ↓
세션 회수 선택
  ┌─────────────────────────────────────┐
  │ 세션 회수 선택                      │
  ├─────────────────────────────────────┤
  │ ○ 1회 세션                          │
  │    - 단일 세션으로 진행             │
  │    - 첫 방문 또는 단기 상담 시 권장 │
  │                                     │
  │ ○ 4회 세션 (4주 과정)               │
  │    - 주 1회, 4주간 진행             │
  │    - 10% 할인 적용                  │
  │                                     │
  │ ○ 8회 세션 (8주 과정)               │
  │    - 주 1회, 8주간 진행             │
  │    - 15% 할인 적용                  │
  │    - 집중 치료 시 권장              │
  │                                     │
  │ ○ 12회 세션 (12주 과정)             │
  │    - 주 1회, 12주간 진행            │
  │    - 20% 할인 적용                  │
  │    - 장기 치료 시 권장              │
  └─────────────────────────────────────┘
  ↓
첫 번째 세션 일시 선택
  "선택하신 회수에 맞춰 향후 일정을 조율할 수 있습니다"
  ↓
[시스템 자동 처리]
  - 선택한 시간 임시 예약 (10분간 유지)
  - 해당 시간 다른 사용자에게 비노출
  - 세션 시작 시간부터 1시간 블록
    (예: 14:00 예약 → 14:00, 15:00 모두 블록)
  ↓
예약 신청 완료
  "예약 신청이 완료되었습니다"
  "10분 내 결제를 완료해주세요"
  "미결제 시 예약이 자동 취소됩니다"
```

#### 4.2 치료사 - 예약 확인 (자동 알림)

```
치료사 대시보드
  ↓
새 예약 알림 (자동)
  "새로운 예약 신청이 들어왔습니다"
  ↓
예약 상세 확인
  ┌─────────────────────────────────────┐
  │ 새 예약 정보                        │
  ├─────────────────────────────────────┤
  │ 예약 일시: 2025-10-05 (토) 14:00   │
  │                                     │
  │ 아이: OO이 (24개월, 남아)           │
  │ 발달 체크 결과:                     │
  │  - 언어: 주의 (45점)                │
  │  - 인지: 양호 (72점)                │
  │  - 기타 영역: 우수                  │
  │                                     │
  │ 방문 주소: 서울시 강남구 XX동       │
  │ (내 위치에서 15분 거리)             │
  │                                     │
  │ 특이사항: [부모가 작성한 내용]      │
  │                                     │
  │ 예약 상태: 결제 대기 중             │
  └─────────────────────────────────────┘
  ↓
캘린더에 자동 표시
  - 예약 시간 블록 표시
  - 결제 완료 시 확정 표시
```

#### 4.3 부모 - 결제 프로세스 (10분 이내)

```
예약 신청 완료 직후
  ↓
결제 화면으로 자동 이동
  ┌─────────────────────────────────────┐
  │ 결제 정보                           │
  ├─────────────────────────────────────┤
  │ 치료사: OOO                         │
  │ 첫 세션 일시: 2025-10-05 (토) 14:00│
  │ 장소: 서울시 강남구 XX동 (우리집)   │
  │ 세션 시간: 50분                     │
  │                                     │
  │ 선택한 회수: 8회 세션 (8주 과정)    │
  │                                     │
  │ ───────────────────────────────────│
  │ 세션 비용 (1회): 80,000원           │
  │ 총 세션 비용 (8회): 640,000원       │
  │ 할인 (15%): -96,000원              │
  │ 할인 후 세션 비용: 544,000원        │
  │                                     │
  │ 플랫폼 수수료 (15%): 81,600원      │
  │ ───────────────────────────────────│
  │ 총 결제 금액: 625,600원             │
  │                                     │
  │ ⏱️ 남은 시간: 09:45               │
  │ ⚠️ 10분 이내 결제하지 않으면       │
  │    예약이 자동 취소됩니다           │
  │                                     │
  │ 📌 2회차 이후 일정은 치료사와      │
  │    상담 후 조율 가능합니다          │
  └─────────────────────────────────────┘
  ↓
[결제하기] 버튼
  ↓
결제 수단 선택
  ┌─────────────────────────────────────┐
  │ 결제 방법 선택                      │
  ├─────────────────────────────────────┤
  │ ○ 신용/체크카드                     │
  │ ○ 계좌이체                          │
  │ ○ 간편결제 (네이버페이, 카카오페이)│
  │ ○ 휴대폰 결제                       │
  └─────────────────────────────────────┘
  ↓
결제 정보 입력
  ↓
[625,600원 결제하기] 클릭
  ↓
PG사 결제 모듈 (토스페이먼츠/아임포트)
  ↓
결제 완료
  ┌─────────────────────────────────────┐
  │ 결제가 완료되었습니다!              │
  ├─────────────────────────────────────┤
  │ 결제 금액: 625,600원                │
  │ 결제 일시: 2025-09-28 15:30        │
  │                                     │
  │ 총 세션 회수: 8회                   │
  │ 첫 세션 일시: 2025-10-05 (토) 14:00│
  │                                     │
  │ ⚠️ 취소/환불 정책                  │
  │ - 각 세션별로 개별 취소 가능        │
  │ - 48시간 전: 해당 세션 전액 환불    │
  │ - 24-48시간 전: 해당 세션 50% 환불  │
  │ - 24시간 이내: 환불 불가            │
  │                                     │
  │ 📌 남은 세션 수: 8회                │
  └─────────────────────────────────────┘
  ↓
예약 확정 완료
  - 부모에게 예약 확정 알림
  - 치료사에게 결제 완료 알림
  - 캘린더에 자동 등록
```

#### 4.4 세션 진행 및 완료

```
세션 당일
  ↓
[세션 D-1] 리마인드 알림
  - 부모: "내일 OOO 치료사 세션이 있습니다"
  - 치료사: "내일 OO이 세션이 예정되어 있습니다"
  ↓
[세션 2시간 전] 알림
  - 부모: "2시간 후 세션이 시작됩니다. 준비 부탁드립니다"
  - 치료사: "방문 출발 시간을 확인해주세요"
  ↓
세션 시작 시간
  ↓
치료사 앱에서 [세션 시작] 버튼 활성화
  ↓
[세션 시작] 클릭
  - 위치 인증 (GPS로 방문 주소 확인)
  - 세션 타이머 시작 (50분)
  ↓
세션 진행
  - 치료사: 아이 평가 및 치료 진행
  - 부모: 세션 참관 가능
  ↓
세션 종료 (50분 경과)
  ↓
치료사 앱에서 [세션 완료] 버튼
  ↓
세션 완료 처리
  - 세션 종료 시간 기록
  - 부모와 치료사에게 완료 알림
```

#### 4.5 세션 후 피드백 및 리뷰

```
세션 완료 후
  ↓
치료사 - 상담일지 작성 (필수, 정산 조건)
  ┌─────────────────────────────────────┐
  │ 상담일지 작성                       │
  ├─────────────────────────────────────┤
  │ ⚠️ 상담일지 작성 완료 시            │
  │    해당 세션 정산이 진행됩니다      │
  │                                     │
  │ 세션 정보:                          │
  │ - 아이: OO이 (24개월, 남아)         │
  │ - 세션 회차: 1/8회                  │
  │ - 세션 일시: 2025-10-05 14:00      │
  │ - 세션 시간: 50분                   │
  │                                     │
  │ ───────────────────────────────────│
  │                                     │
  │ 아이 상태 평가:                     │
  │ [언어 발달 상태에 대한 평가]        │
  │ (필수 입력)                         │
  │                                     │
  │ 세션 내용:                          │
  │ [진행한 활동 및 치료 내용]          │
  │ (필수 입력)                         │
  │                                     │
  │ 가정 과제:                          │
  │ [가정에서 할 수 있는 활동]          │
  │ (필수 입력)                         │
  │                                     │
  │ 다음 세션 목표:                     │
  │ [다음 회차 진행 계획]               │
  │ (선택 입력)                         │
  │                                     │
  │ 추천 놀이영상:                      │
  │ [관련 영상 선택 - 선택사항]         │
  │                                     │
  │ ───────────────────────────────────│
  │                                     │
  │ 다음 세션 일정 제안:                │
  │ ○ 2025-10-12 (토) 14:00            │
  │ ○ 2025-10-12 (토) 15:00            │
  │ ○ 부모와 별도 조율                  │
  │                                     │
  │ [상담일지 제출하기]                 │
  │ ⚠️ 제출 후 수정 불가               │
  └─────────────────────────────────────┘
  ↓
상담일지 제출
  ↓
[시스템 자동 처리]
  - 상담일지 작성 완료 확인
  - 해당 세션 정산 대상으로 등록
  - 부모에게 상담일지 공유
  - 다음 세션 일정 알림 (복수회차인 경우)
  ↓
부모 - 상담일지 확인
  ┌─────────────────────────────────────┐
  │ 상담일지 (1/8회차)                  │
  ├─────────────────────────────────────┤
  │ 치료사: OOO                         │
  │ 세션 일시: 2025-10-05 14:00        │
  │                                     │
  │ [아이 상태 평가]                    │
  │ [세션 내용]                         │
  │ [가정 과제]                         │
  │ [다음 세션 목표]                    │
  │                                     │
  │ 다음 세션 일정 제안:                │
  │ - 2025-10-12 (토) 14:00            │
  │                                     │
  │ [일정 확인하기]                     │
  │ [리뷰 작성하기]                     │
  └─────────────────────────────────────┘
  ↓
다음 세션 일정 확정 (복수회차인 경우)
  - 부모가 제안된 일정 선택 또는 별도 조율
  - 치료사 캘린더에 자동 반영
  ↓
부모 - 리뷰 작성 (선택)
  ┌─────────────────────────────────────┐
  │ 치료사 리뷰 작성                    │
  ├─────────────────────────────────────┤
  │ 평점: ⭐⭐⭐⭐⭐ (5.0)              │
  │                                     │
  │ 만족도:                             │
  │ - 전문성: ⭐⭐⭐⭐⭐                │
  │ - 친절도: ⭐⭐⭐⭐⭐                │
  │ - 시간 준수: ⭐⭐⭐⭐⭐             │
  │                                     │
  │ 리뷰 내용:                          │
  │ [자유 작성]                         │
  │                                     │
  │ 재예약 의향:                        │
  │ ○ 예 (바로 예약하기 가능)           │
  │ ○ 아니오                            │
  └─────────────────────────────────────┘
  ↓
리뷰 제출 → 치료사 프로필에 반영
```

#### 4.6 세션 일정 변경 프로세스

```
부모 - 세션 일정 변경 요청
  ↓
마이페이지 > 예약 내역
  ┌─────────────────────────────────────┐
  │ 예약된 세션 목록                    │
  ├─────────────────────────────────────┤
  │ 치료사: OOO                         │
  │ 세션 회차: 3/8회                    │
  │                                     │
  │ 예정 세션:                          │
  │ - 2025-10-12 (토) 14:00            │
  │                                     │
  │ [일정 변경 요청]                    │
  └─────────────────────────────────────┘
  ↓
[일정 변경 요청] 클릭
  ↓
변경 요청 작성
  ┌─────────────────────────────────────┐
  │ 일정 변경 요청                      │
  ├─────────────────────────────────────┤
  │ 현재 예약: 2025-10-12 (토) 14:00   │
  │                                     │
  │ 변경 사유:                          │
  │ [사유 작성]                         │
  │ 예) 가족 행사, 아이 컨디션 등       │
  │                                     │
  │ 희망 일정 (1-3순위):                │
  │ 1순위: 2025-10-13 (일) 10:00       │
  │ 2순위: 2025-10-14 (월) 15:00       │
  │ 3순위: 2025-10-15 (화) 14:00       │
  │                                     │
  │ ⚠️ 취소/환불 정책 적용:             │
  │ - 48시간 전: 무료 변경 가능         │
  │ - 48시간 이내: 치료사 승인 필요     │
  └─────────────────────────────────────┘
  ↓
[변경 요청 보내기]
  ↓
시스템 자동 처리
  - 치료사에게 변경 요청 알림
  - 요청 상태: 승인 대기
  ↓
치료사 - 변경 요청 확인 및 승인
  ┌─────────────────────────────────────┐
  │ 일정 변경 요청                      │
  ├─────────────────────────────────────┤
  │ 요청자: OO 부모님                   │
  │ 아이: OO이 (3/8회차)                │
  │                                     │
  │ 현재 예약: 2025-10-12 (토) 14:00   │
  │                                     │
  │ 변경 사유:                          │
  │ [부모가 작성한 사유]                │
  │                                     │
  │ 희망 일정:                          │
  │ ○ 1순위: 2025-10-13 (일) 10:00     │
  │   상태: 예약 가능 ✅                │
  │                                     │
  │ ○ 2순위: 2025-10-14 (월) 15:00     │
  │   상태: 예약 불가 ❌ (다른 예약 있음)│
  │                                     │
  │ ○ 3순위: 2025-10-15 (화) 14:00     │
  │   상태: 예약 가능 ✅                │
  │                                     │
  │ ○ 다른 날짜 제안하기                │
  │                                     │
  │ [승인] [거절]                       │
  └─────────────────────────────────────┘
  ↓
치료사 결정
  ↓
┌────────────────────────────────────────┐
│ [승인하는 경우]                        │
│                                        │
│ 희망 일정 선택 또는 다른 날짜 제안     │
│   ↓                                    │
│ 일정 변경 확정                         │
│   - 기존 예약 취소                     │
│   - 새 일정으로 예약 생성              │
│   - 부모에게 변경 확정 알림            │
│   ↓                                    │
│ 변경 완료                              │
│   "일정이 변경되었습니다"              │
│   "새 일정: 2025-10-13 (일) 10:00"    │
└────────────────────────────────────────┘
  또는
┌────────────────────────────────────────┐
│ [거절하는 경우]                        │
│                                        │
│ 거절 사유 작성                         │
│ [예시]                                 │
│ - 제시된 날짜 모두 예약 불가           │
│ - 다른 날짜 조율 필요                  │
│   ↓                                    │
│ 부모에게 거절 알림                     │
│   - 거절 사유 전달                     │
│   - 재조율 요청                        │
└────────────────────────────────────────┘
```

#### 4.7 회사 → 치료사 정산 프로세스

```
[매주 월요일 정산 처리]

시스템 자동 정산 계산
  ↓
  지난주 완료 + 상담일지 작성 완료된 세션 조회
  ⚠️ 상담일지 미작성 세션은 정산 대상에서 제외
  ┌─────────────────────────────────────┐
  │ 정산 대상 세션                      │
  ├─────────────────────────────────────┤
  │ 치료사: OOO                         │
  │                                     │
  │ 2025-09-23 세션 (상담일지 작성완료):│
  │ - 세션 비용: 80,000원               │
  │ - 플랫폼 수수료(15%): -12,000원    │
  │ - 정산 금액: 68,000원               │
  │                                     │
  │ 2025-09-25 세션 (상담일지 작성완료):│
  │ - 세션 비용: 80,000원               │
  │ - 플랫폼 수수료(15%): -12,000원    │
  │ - 정산 금액: 68,000원               │
  │                                     │
  │ 2025-09-27 세션 (복수회차 8회 중):  │
  │ - 할인 적용 세션 비용: 68,000원     │
  │ - 플랫폼 수수료(15%): -10,200원    │
  │ - 정산 금액: 57,800원               │
  │                                     │
  │ ...                                 │
  │                                     │
  │ ⚠️ 2025-09-24 세션:                │
  │    상담일지 미작성으로 정산 보류    │
  │                                     │
  │ 총 5건 세션 완료 (1건 보류)         │
  │ ───────────────────────────────────│
  │ 총 세션 비용: 376,000원             │
  │ 플랫폼 수수료: -56,400원            │
  │ ───────────────────────────────────│
  │ 정산 금액: 319,600원                │
  │                                     │
  │ 원천징수세(3.3%): -10,547원        │
  │ ───────────────────────────────────│
  │ 실지급액: 309,053원                 │
  └─────────────────────────────────────┘
  ↓
치료사에게 정산 내역 알림
  "이번 주 정산 금액이 확정되었습니다"
  ↓
치료사 - 정산 내역 확인
  ┌─────────────────────────────────────┐
  │ 정산 대시보드                       │
  ├─────────────────────────────────────┤
  │ 이번 주 정산 (2025-09-23 ~ 09-29) │
  │                                     │
  │ 완료 세션: 6건                      │
  │ 정산 대상: 5건 (상담일지 작성완료)  │
  │ 정산 보류: 1건 (상담일지 미작성)    │
  │                                     │
  │ 총 세션 비용: 376,000원             │
  │ 플랫폼 수수료: -56,400원            │
  │ 원천징수: -10,547원                 │
  │ 실지급액: 309,053원                 │
  │                                     │
  │ ⚠️ 정산 보류 세션:                 │
  │ - 2025-09-24 세션 (상담일지 미작성)│
  │   → 상담일지 작성 시 다음 주 정산  │
  │                                     │
  │ 지급 예정일: 2025-10-02 (수)       │
  │ 지급 계좌: XX은행 1234-56-789012    │
  │                                     │
  │ [상세 내역 보기]                    │
  │ [보류 세션 상담일지 작성하기]       │
  │ [세금계산서 발급]                   │
  └─────────────────────────────────────┘
  ↓
[정산 지급일]
  ↓
회사 → 치료사 계좌로 자동 이체
  ↓
치료사 - 입금 완료 알림
  "정산 금액 309,053원이 입금되었습니다"
  "(5건 세션, 1건 보류)"
  ↓
정산 완료
  - 정산 이력 저장
  - 세금계산서 자동 발급
  - 보류된 세션은 상담일지 작성 후 다음 주 정산
```

#### 4.8 예외 처리 - 취소 및 환불

```
부모가 세션 취소 요청
  ↓
취소 시점 확인
  ↓
┌─────────────────────────────────────┐
│ [48시간 전 취소]                    │
│ - 전액 환불 (92,000원)              │
│ - 치료사에게 취소 알림              │
│ - 수수료 없음                       │
└─────────────────────────────────────┘
  ↓
┌─────────────────────────────────────┐
│ [24-48시간 전 취소]                 │
│ - 50% 환불 (46,000원)               │
│ - 치료사에게 보상금 지급 (23,000원) │
│ - 회사 수수료: 23,000원             │
└─────────────────────────────────────┘
  ↓
┌─────────────────────────────────────┐
│ [24시간 이내 취소]                  │
│ - 환불 불가                         │
│ - 치료사 전액 지급 (80,000원)       │
│ - 회사 수수료: 12,000원             │
│ - 부모에게 환불 불가 안내           │
└─────────────────────────────────────┘
  ↓
환불 처리 (3-5 영업일)
```

**주요 화면:**
- 치료사 검색 및 필터링 화면
- 치료사 프로필 상세 화면
- 방문 세션 신청 화면
- 결제 화면
- 예약 확정 화면
- 세션 진행 화면 (치료사 앱)
- 피드백 작성 화면
- 리뷰 작성 화면
- 정산 대시보드 (치료사)
- 취소/환불 화면

**알림:**
- 매칭 승인/거절 알림
- 결제 요청 알림 (48시간 전까지)
- 결제 완료 알림
- 세션 D-1 리마인드 알림
- 세션 2시간 전 알림
- 세션 완료 알림
- 피드백 작성 요청 알림
- 정산 확정 알림
- 정산 입금 완료 알림

### 5. 마이페이지 플로우

```
마이페이지 진입
  ↓
┌─────────────────────────────────────────────┐
│ 메뉴                                        │
├─────────────────────────────────────────────┤
│ - 내 정보 관리                              │
│ - 발달 기록                                 │
│ - 추천 영상 (NEW)                           │
│ - 나의 활동                                 │
│ - 상담 이력                                 │
│ - 고객 지원                                 │
└─────────────────────────────────────────────┘

├──→ [내 정보 관리]
│      - 프로필 수정
│      - 아이 정보 수정/추가
│      - 알림 설정
│
├──→ [발달 기록]
│      - 전체 발달 체크 이력
│      - 시간별 발달 그래프
│      - 영역별 성장 추이
│
├──→ [추천 영상] ⭐ NEW
│      ↓
│      추천 영상 화면
│      ┌─────────────────────────────────────┐
│      │ OO이를 위한 맞춤 추천 영상          │
│      │ (최신 발달 체크 결과 기반)          │
│      ├─────────────────────────────────────┤
│      │ 마지막 체크: 2025-09-20             │
│      │ 다음 체크 권장일: 2025-10-20        │
│      │ [새로 체크하기]                     │
│      └─────────────────────────────────────┘
│      ↓
│      ┌─────────────────────────────────────┐
│      │ 🟠 주의 영역: 언어 발달             │
│      │    집중 추천 영상 (10개)            │
│      ├─────────────────────────────────────┤
│      │ [영상1] 말놀이 동요 따라하기        │
│      │ [영상2] 그림책 읽어주기             │
│      │ [영상3] 소리 듣고 따라하기          │
│      │ ...                                 │
│      │ [더보기]                            │
│      └─────────────────────────────────────┘
│      ↓
│      ┌─────────────────────────────────────┐
│      │ 🟡 양호 영역: 인지 발달             │
│      │    발달 유지 영상 (5개)             │
│      ├─────────────────────────────────────┤
│      │ [영상4] 색깔 맞추기 놀이            │
│      │ [영상5] 모양 찾기 게임              │
│      │ ...                                 │
│      │ [더보기]                            │
│      └─────────────────────────────────────┘
│      ↓
│      ┌─────────────────────────────────────┐
│      │ 🟢 우수 영역: 대근육, 사회성, 정서  │
│      │    심화 학습 영상 (5개)             │
│      ├─────────────────────────────────────┤
│      │ [영상6] 신나는 체조 시간            │
│      │ [영상7] 친구와 함께하는 놀이        │
│      │ ...                                 │
│      │ [더보기]                            │
│      └─────────────────────────────────────┘
│      ↓
│      영상 클릭 시 → 영상 상세 화면으로 이동
│                   → 바로 재생 가능
│
├──→ [나의 활동]
│      - 북마크한 영상
│      - 시청 기록
│      - 완료한 놀이
│
├──→ [상담 이력]
│      - 예정된 상담
│      - 완료된 상담
│      - 치료사 피드백
│
└──→ [고객 지원]
       - FAQ
       - 1:1 문의
       - 공지사항
```

**추천 영상 코너 주요 기능:**
- 최신 발달 체크 결과 기반 실시간 추천
- 발달 단계별 영상 그룹핑
  - 주의 영역: 집중 추천 (많은 영상)
  - 양호 영역: 유지 추천
  - 우수 영역: 심화 추천
- 체크 이력이 없으면 연령 기반 추천
- 영상 클릭 시 바로 재생 가능

---

## 치료사 플로우

### 1. 치료사 회원가입 및 인증

```
시작
  ↓
치료사 회원가입 선택
  ↓
기본 정보 입력
  - 이름
  - 연락처
  - 이메일
  ↓
전문가 정보 입력
  - 치료 분야
  - 자격증 (사진 업로드)
  - 경력 연차
  - 소속 기관 (있는 경우)
  ↓
프로필 작성
  - 전문 분야 상세
  - 치료 철학/소개
  - 상담 가능 지역
  - 상담 방식 (방문/센터/온라인)
  - 상담 비용
  - 상담 가능 시간
  ↓
서류 제출
  - 자격증 사본
  - 신분증 사본
  - 사업자등록증 (있는 경우)
  ↓
관리자 검토 대기
  ↓
[승인 시] 치료사 계정 활성화
[거절 시] 사유 안내 및 재신청 안내
```

**주요 화면:**
- 치료사 회원가입 화면
- 프로필 작성 화면
- 서류 제출 화면
- 승인 대기 화면

### 2. 매칭 요청 관리 플로우

```
치료사 대시보드
  ↓
새 매칭 요청 알림 확인
  ↓
매칭 요청 목록 확인
  - 대기 중인 요청
  - 승인한 요청
  - 거절한 요청
  ↓
요청 상세 확인
  - 아이 기본 정보 (나이, 성별)
  - 발달 체크 결과 요약
  - 부모가 작성한 특이사항
  - 상담 희망 일시
  ↓
[승인] 또는 [거절] 선택
  ↓
[승인 시]
  - 상담 일시 확정 또는 조율
  - 준비사항 안내 메시지 전송
  ↓
[거절 시]
  - 거절 사유 선택 (일정 불가, 전문분야 불일치 등)
  ↓
확정된 예약 관리
  - 예약 캘린더 확인
  - 예약 상세 정보
  - 사전 준비 자료 확인
```

**주요 화면:**
- 치료사 대시보드
- 매칭 요청 목록
- 요청 상세 화면
- 예약 캘린더

**알림:**
- 새 매칭 요청 알림
- 상담 D-1 알림
- 상담 30분 전 알림

### 3. 상담 및 피드백 플로우

```
상담 당일
  ↓
[상담 시작] 버튼 활성화
  ↓
상담 진행
  - 온라인 상담: 화상 통화 연결
  - 오프라인 상담: 현장 진행
  ↓
상담 중 메모 작성 (선택)
  ↓
[상담 완료] 처리
  ↓
상담 후 피드백 작성
  - 아이 상태 평가
  - 권장 사항
  - 가정에서 할 수 있는 놀이/활동
  - 다음 상담 필요 여부
  ↓
피드백 전송
  ↓
부모가 피드백 확인
  ↓
부모 리뷰 받기
```

**주요 화면:**
- 상담 진행 화면 (화상 통화 연동)
- 상담 메모 화면
- 피드백 작성 화면
- 완료된 상담 이력

### 4. 프로필 및 일정 관리

```
치료사 마이페이지
  ↓
프로필 수정
  - 소개 문구 변경
  - 전문 분야 업데이트
  - 상담 비용 조정
  ↓
일정 관리
  - 상담 가능 시간 설정
  - 휴무일 등록
  - 일정 블록 설정
  ↓
수익 관리
  - 월별 상담 현황
  - 수익 내역
  - 정산 신청
  ↓
리뷰 관리
  - 받은 리뷰 확인
  - 리뷰 답글 작성
```

---

## 관리자 플로우

### 1. 콘텐츠 관리 플로우

#### 1.1 발달 체크리스트 관리

```
관리자 대시보드
  ↓
[발달 체크 관리] 메뉴
  ↓
연령대별 체크리스트 확인
  - 0-6개월
  - 7-12개월
  - 13-18개월
  - ...
  - 72개월 이상
  ↓
체크리스트 수정
  - 질문 추가/삭제/수정
  - 평가 기준 조정
  - 영역별 가중치 설정
  ↓
전문가 검토 요청 (선택)
  ↓
변경사항 적용
  ↓
버전 관리 (이전 버전 보관)
```

#### 1.2 놀이영상 관리

```
[놀이영상 관리] 메뉴
  ↓
새 영상 업로드
  - 영상 파일 업로드
  - 썸네일 이미지
  ↓
영상 정보 입력
  - 제목
  - 설명
  - 대상 연령
  - 발달 영역 태그
  - 준비물
  - 놀이 시간
  - 난이도
  - 무료/프리미엄 구분
  ↓
카테고리 분류
  ↓
미리보기 및 검수
  ↓
게시 또는 예약 게시
  ↓
영상 통계 확인
  - 조회수
  - 북마크 수
  - 완료 체크 수
  - 평균 시청 시간
```

**주요 기능:**
- 영상 일괄 업로드
- 영상 편집 (자막, 구간 설정)
- 시리즈 영상 관리
- 영상 성과 분석

### 2. 사용자 관리 플로우

```
[사용자 관리] 메뉴
  ↓
사용자 목록 확인
  - 전체 사용자
  - 부모 사용자
  - 치료사
  ↓
필터 및 검색
  - 가입일
  - 구독 상태
  - 활동 상태
  ↓
사용자 상세 정보
  - 기본 정보
  - 활동 이력
  - 결제 이력
  - 고객 문의 이력
  ↓
계정 관리
  - 계정 정지
  - 경고 발송
  - 메모 작성
```

### 3. 치료사 인증 관리 플로우

```
[치료사 인증] 메뉴
  ↓
대기 중인 신청 목록
  ↓
신청서 상세 확인
  - 제출 서류 검토
  - 자격증 진위 확인
  - 경력 확인
  ↓
추가 서류 요청 (필요 시)
  ↓
[승인] 또는 [거절]
  ↓
[승인 시]
  - 치료사 계정 활성화
  - 승인 완료 이메일 발송
  ↓
[거절 시]
  - 거절 사유 작성
  - 재신청 안내
```

### 4. 매칭 및 상담 관리

```
[매칭 관리] 메뉴
  ↓
전체 매칭 현황 확인
  - 매칭 요청 수
  - 승인율
  - 상담 완료율
  ↓
문제 상황 모니터링
  - 미응답 치료사
  - 반복 거절 사례
  - 상담 취소 사례
  ↓
분쟁 조정
  - 환불 요청 처리
  - 불만 사항 중재
```

### 5. 통계 및 리포트

```
[통계 대시보드] 메뉴
  ↓
주요 지표 확인
  - DAU/MAU
  - 신규 가입자 수
  - 발달 체크 완료 수
  - 영상 재생 수
  - 매칭 성사 수
  - 구독 전환율
  ↓
상세 분석
  - 연령대별 사용 패턴
  - 인기 영상 TOP 10
  - 지역별 치료사 분포
  - 이탈률 분석
  ↓
리포트 생성
  - 주간/월간 리포트
  - 맞춤 리포트 (기간, 지표 선택)
  ↓
리포트 내보내기 (PDF, Excel)
```

---

## 시스템 간 연동 플로우

### 1. 발달 체크 → 놀이영상 추천

```
사용자가 발달 체크 완료
  ↓
결과 분석 엔진 작동
  ↓
주의 필요 영역 식별
  ↓
해당 영역 집중 놀이영상 필터링
  ↓
아이 연령에 맞는 영상 선별
  ↓
추천 영상 리스트 생성
  ↓
발달 리포트에 표시
  ↓
푸시 알림: "OO이에게 딱 맞는 놀이영상을 확인하세요"
```

### 2. 발달 체크 → 치료사 매칭

```
발달 체크 결과 분석
  ↓
발달 지연 의심 영역 감지
  ↓
[조건: 특정 영역 점수가 하위 10%]
  ↓
적합한 치료 분야 식별
  - 언어 점수 낮음 → 언어치료
  - 대근육 점수 낮음 → 물리치료/작업치료
  ↓
사용자에게 권장 알림 표시
  "언어 발달을 위해 전문가 상담을 권장합니다"
  ↓
[치료사 찾기] 버튼 노출
  ↓
클릭 시 해당 분야 치료사 목록으로 자동 이동
  ↓
AI 추천 치료사 우선 표시
```

### 3. 치료사 피드백 → 놀이영상 추천

```
치료사가 상담 후 피드백 작성
  ↓
"가정에서 할 수 있는 활동" 항목에 키워드 입력
  예: "언어 자극", "소근육 발달"
  ↓
시스템이 키워드 매칭
  ↓
관련 놀이영상 자동 추천
  ↓
부모가 피드백 확인 시 추천 영상도 함께 표시
  ↓
[영상 바로보기] 버튼으로 즉시 시청 가능
```

### 4. 정기 체크 알림 자동화

```
사용자가 마지막 발달 체크 완료
  ↓
시스템에 완료 날짜 저장
  ↓
[3개월 후]
  ↓
정기 체크 알림 발송
  - 푸시 알림
  - 이메일 알림
  ↓
사용자가 알림 확인 및 체크 시작
  ↓
[미완료 시]
  ↓
1주일 후 리마인드 알림
  ↓
[계속 미완료 시]
  ↓
2주일 후 마지막 알림
```

### 5. 구독 관리 자동화

```
무료 사용자가 프리미엄 영상 클릭
  ↓
"프리미엄 구독이 필요합니다" 안내
  ↓
[구독하기] 버튼
  ↓
구독 플랜 선택
  - 월간 9,900원
  - 연간 99,000원 (2개월 무료)
  ↓
결제 수단 입력
  ↓
결제 완료
  ↓
즉시 프리미엄 기능 활성화
  ↓
[구독 만료 3일 전]
  ↓
갱신 안내 알림
  ↓
자동 갱신 또는 구독 취소
```

### 6. 데이터 분석 파이프라인

```
사용자 활동 데이터 수집
  - 발달 체크 결과
  - 영상 시청 기록
  - 매칭 신청 및 상담 이력
  ↓
데이터 웨어하우스 저장
  ↓
일간 배치 작업 실행
  ↓
분석 결과 생성
  - 사용자별 참여도 점수
  - 이탈 위험 사용자 식별
  - 인기 콘텐츠 순위
  ↓
결과를 관리자 대시보드에 반영
  ↓
마케팅 자동화 트리거
  - 이탈 위험 사용자에게 할인 쿠폰 발송
  - 인기 영상 소셜 미디어 홍보
```

---

## 주요 예외 처리 플로우

### 1. 발달 체크 중단 시

```
사용자가 발달 체크 중 이탈
  ↓
진행 상황 자동 저장 (마지막 응답까지)
  ↓
24시간 후 "이어서 하기" 알림 발송
  ↓
[재진입 시] 이어서 하기 옵션 제공
  ↓
[7일 이상 미완료] 데이터 삭제 또는 보관
```

### 2. 매칭 무응답 시

```
부모가 치료사에게 매칭 신청
  ↓
치료사 48시간 이내 미응답
  ↓
치료사에게 리마인드 알림 발송
  ↓
[24시간 추가 대기]
  ↓
여전히 무응답
  ↓
부모에게 "다른 치료사 추천" 알림
  ↓
대체 치료사 목록 제공
  ↓
무응답 치료사 프로필에 응답률 반영
```

### 3. 상담 취소 시

```
부모 또는 치료사가 상담 취소 요청
  ↓
[상담 24시간 전 취소]
  - 수수료 없이 취소 가능
  - 상대방에게 취소 알림
  ↓
[상담 24시간 이내 취소]
  - 취소 수수료 부과 안내
  - 사유 확인
  - 긴급 상황일 경우 예외 처리
  ↓
취소 확정
  ↓
환불 처리 (결제한 경우)
  ↓
양측에 취소 완료 알림
```

### 4. 결제 실패 시

```
구독 자동 갱신 시도
  ↓
결제 실패 (카드 한도 초과, 만료 등)
  ↓
결제 실패 알림 발송
  "결제가 실패했습니다. 결제 수단을 확인해주세요"
  ↓
3일 유예 기간 제공
  ↓
재시도 실패 시
  ↓
구독 자동 해지
  ↓
프리미엄 기능 비활성화
  ↓
무료 플랜으로 자동 전환
```

---

## 플로우 최적화 포인트

### 사용자 경험 최적화
1. **원클릭 액션**: 발달 리포트에서 놀이영상/치료사 매칭으로 바로 이동
2. **진행 상황 저장**: 모든 중요 작업에서 중간 저장 기능
3. **스마트 알림**: 사용자 활동 패턴 학습 후 최적 시간에 알림 발송
4. **빠른 재시작**: 중단했던 작업을 메인 화면에서 바로 재개

### 치료사 효율성 최적화
1. **일괄 처리**: 여러 매칭 요청을 한 번에 처리 가능
2. **캘린더 연동**: 구글 캘린더, 애플 캘린더 연동
3. **템플릿 기능**: 자주 쓰는 피드백 문구 템플릿 저장
4. **모바일 최적화**: 이동 중에도 요청 확인 및 승인 가능

### 관리자 운영 최적화
1. **자동 필터링**: 부적절한 콘텐츠 자동 감지
2. **배치 작업**: 대량의 데이터 처리 자동화
3. **이상 징후 알림**: 급격한 이탈률 증가, 서버 오류 등 즉시 알림
4. **A/B 테스트**: UI 변경사항 실험 및 효과 측정